...
## This project (Commercial Dispute Expert)

| Setting | Value |
|---------|--------|
| **`BRAND_NAME`** | `Commercial Dispute Expert` |
| **`domain`** (when `NEXT_PUBLIC_SITE_URL` is set) | `commercialdisputeexpert.com` |
| Contact webhook | **`/api/submit-lead`** primary on form submit |
| Google Sheets | Soft-fail via **`/api/instruct`** — one shared tab + **Form Type** (Contact / Instruct) |
